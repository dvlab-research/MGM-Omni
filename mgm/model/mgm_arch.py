...
from mgm.model.multimodal_encoder.builder import build_vision_tower, build_speech_tower
from mgm.model.multimodal_projector.builder import build_vision_projector, build_speech_projector
from mgm.constants import (IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, SPEECH_TOKEN_INDEX,
                           DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN)
from mgm.model.soft_dtw_cuda import SoftDTW
...
class MGMMetaModel:
    ...
    def initialize_speech_modules(self, model_args, fsdp=None):  
        ...
        elif 'whisper' or 'qwen2a' in model_args.speech_tower.lower():
...
class MGMMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, is_video=False):        
        if "qwen2.5-vl" in self.config.mm_vision_tower.lower():
    ...
    def encode_speeches_beats(self, speeches, wavs):
        if isinstance(speeches, list):
            if len(speeches[0].shape) == 2:
                speeches = torch.stack(speeches, dim=0)
                wavs = torch.stack(wavs, dim=0)
                speech_features = self.get_model().get_speech_tower()(speeches, wavs)
                speech_features = self.get_model().mm_speech_projector(speech_features)
                if self.config.speech_norm:
                    speech_features = torch.nn.functional.normalize(speech_features, dim=-1)
                return speech_features
            ...
            if len(speeches[0].shape) == 3:
                speech_features = []
                for speech, wav in zip(speeches, wavs):
                    speech_feature = self.get_model().get_speech_tower()(speech, wav)
                    speech_feature = self.get_model().mm_speech_projector(speech_feature)
                    if self.config.speech_norm:
                        speech_feature = torch.nn.functional.normalize(speech_feature, dim=-1)
                    speech_features.append(speech_feature.flatten(0, 1).contiguous())
    ...
    def prepare_inputs_labels_for_text_image_speech_qwen2vl(
        ...
        if speech_tower is not None and speeches is not None:
            ...
            if self.training and getattr(self.config, 'asr_align', False):
            ...
            for i in range(num_mm + 1):
                ...
                if i < num_mm:
                    if cur_input_ids[mm_token_indices[i+1]] == IMAGE_TOKEN_INDEX:
                        ...
                        if hasattr(images[cur_im_idx], 'image_grid_thw'):
                            if self.training:
                                t, h, w = images[cur_im_idx]['image_grid_thw'][0]
                            else:
                                if len(images[cur_im_idx]['image_grid_thw'].shape) == 3:
                                    t = images[cur_im_idx]['image_grid_thw'][0, 0, 0].item()
                                    h = images[cur_im_idx]['image_grid_thw'][0, 0, 1].item()
                                    w = images[cur_im_idx]['image_grid_thw'][0, 0, 2].item()
                                elif len(images[cur_im_idx]['image_grid_thw'].shape) == 2:
                                    t = images[cur_im_idx]['image_grid_thw'][0, 0].item()
                                    h = images[cur_im_idx]['image_grid_thw'][0, 1].item()
                                    w = images[cur_im_idx]['image_grid_thw'][0, 2].item()
                                else:
                                    t, h, w = images[cur_im_idx]['image_grid_thw'][0]
                        else:
                            if self.training:
                                t, h, w = images[cur_im_idx]['video_grid_thw'][0]
                            else:
                                if len(images[cur_im_idx]['video_grid_thw'].shape) == 3:
                                    t = images[cur_im_idx]['video_grid_thw'][0, 0, 0].item()
                                    h = images[cur_im_idx]['video_grid_thw'][0, 0, 1].item()
                                    w = images[cur_im_idx]['video_grid_thw'][0, 0, 2].item()
                                elif len(images[cur_im_idx]['video_grid_thw'].shape) == 2:
                                    t = images[cur_im_idx]['video_grid_thw'][0, 0].item()
                                    h = images[cur_im_idx]['video_grid_thw'][0, 1].item()
                                    w = images[cur_im_idx]['video_grid_thw'][0, 2].item()
                                else:
                                    t, h, w = images[cur_im_idx]['video_grid_thw'][0]
